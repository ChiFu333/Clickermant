...
public class Grid<T>
{
    ...
    public void SetValueAtWorld(Vector2 position, T value) {
        Vector2Int cellPosition = GetCellAtWorld(position);
        SetValueAt(cellPosition, value);
    }
    #endregion
    #region Getting
    ...
    public bool isBoxValid(Vector2Int position, Vector2Int size) {
        if (position.x < 0 || position.x + size.x > width) return false;
        if (position.y < 0 || position.y + size.y > height) return false;
        return true;
    }
    #endregion
}
